...
def complete_profile(request):
    ...
    if request.method == 'POST':
        ...
        Profile.objects.create(user=request.user, nickname=nickname, username=username)
        return redirect('home')
...
def profile(request, username=None):
    ...
    # Kendi profili mi, yoksa başka bir kullanıcının profili mi?
    if username:
    ...
    # Görünürlük ayarlarını kontrol et
...
def update_visibility(request):
    profile = request.user.profile
    if request.method == 'POST':
        # Checkbox durumlarını açıkça kontrol et
